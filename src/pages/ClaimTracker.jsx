...
export default function ClaimTracker() {
  ...
  const [newConfig, setNewConfig] = useState({ company_name: '', api_url: '', api_key: '', headers: '{}', auth_type: 'API Key', is_active: true });
  ...
  const handleOpenApiConfigOption = () => {
  ...
  const fetchApiConfigs = async () => {
    setApiLoading(true);
    try {
      const res = await api.get('/claim-tracker/admin/api-configs');
  ...
  const parseCompanyName = (url) => {
    try {
      const lower = url.toLowerCase();
      if (lower.includes('tata-aig') || lower.includes('tataaig')) return 'Tata AIG';
      if (lower.includes('hdfc')) return 'HDFC ERGO';
      if (lower.includes('sbi')) return 'SBI General';
      if (lower.includes('icici')) return 'ICICI Lombard';
      ...
      const lower = url.toLowerCase();
      if (lower.includes('tata')) return 'Tata AIG';
      if (lower.includes('hdfc')) return 'HDFC ERGO';
      if (lower.includes('sbi')) return 'SBI General';
      if (lower.includes('icici')) return 'ICICI Lombard';
      return 'External API';
  ...
  const handleSaveApiConfig = async () => {
    try {
      const company = parseCompanyName(newConfig.api_url);
      const payload = {
      ...
      await api.post('/claim-tracker/admin/api-configs', payload);
      fetchApiConfigs();
      setNewConfig({ company_name: '', api_url: '', api_key: '', headers: '{}', auth_type: 'API Key', is_active: true });
    } catch (e) {
      alert('Failed to save API config: ' + (e.response?.data?.detail || e.message));
    }
  };

  const handleDeleteApiConfig = async (id) => {
    if (!window.confirm("Delete this config?")) return;
    try {
      await api.delete(`/claim-tracker/admin/api-configs/${id}`);
      fetchApiConfigs();
    } catch (e) {
    ...
    <Box sx={{ width: '100%', color: themeColors.textPrimary }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          ...
          >
            <MenuItem onClick={handleOpenApiConfigOption}>
              <ListItemIcon sx={{ minWidth: 36 }}>
                <SettingsIcon fontSize="small" sx={{ color: themeColors.textSecondary }} />
              </ListItemIcon>
              <ListItemText primary="API URL Put" />
            </MenuItem>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        {fetchingAll || loading ? (
          ...
          <Paper sx={{ p: 4, textAlign: 'center', bgcolor: themeColors.cardBg, border: themeColors.border }}>
            <Typography sx={{ color: themeColors.textSecondary }}>No claims found. Configure APIs or upload CSV claims to populate.</Typography>
          </Paper>
        ) : (
      ...
      >
        <DialogTitle sx={{ fontWeight: 800, borderBottom: themeColors.border, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <IconButton onClick={() => setOpenApiDialog(false)} sx={{ color: themeColors.textSecondary }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <TextField
              ...
              fullWidth
              size="small"
              placeholder="e.g. http://localhost:8000/claim-tracker/mock-api/hdfc"
            ...
            <Button
              ...
              disabled={!newConfig.api_url.trim()}
              sx={{
                bgcolor: '#2563eb',
                color: '#fff',
                fontWeight: 600,
              ...
              <List dense>
                {apiConfigs.map((cfg) => (
                  <ListItem
                    key={cfg.id}
                    secondaryAction={
                      <IconButton edge="end" aria-label="delete" onClick={() => handleDeleteApiConfig(cfg.id)} sx={{ color: '#ef4444' }}>
                        <DeleteIcon />
                      </IconButton>
                    }
                    sx={{ borderBottom: themeColors.border }}
                  >
                    <ListItemText
                      primary={cfg.company_name}
                      secondary={cfg.api_url}
        ...
        <DialogActions sx={{ p: 2, borderTop: themeColors.border }}>
          <Button onClick={() => setOpenApiDialog(false)} variant="outlined" sx={{ borderRadius: 2 }}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
